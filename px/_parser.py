...
tlist = Keyword("TLIST") + Group(
    BPAREN
    + Word("AHQMW1")
    + (
        (COMMA + QSTRING + Literal("-") + QSTRING + EPAREN)
        | (EPAREN + OneOrMore(QSTRING | COMMA))
    )
)
...
codes = (
    ...
    + EPAREN
    + EQUAL
    + Group(OneOrMore(QSTRING | NEWLINE | COMMA))
    + SEMI
)
contact = (
    Keyword("CONTACT") + Optional(BPAREN + QSTRING + EPAREN) + EQUAL + MLQSTRING + SEMI
)
...
elimination = (
    Keyword("ELIMINATION") + BPAREN + QSTRING + EPAREN + EQUAL + (QSTRING | YES) + SEMI
)
...
notex = (
    Keyword("NOTEX") + Optional(BPAREN + QSTRING + EPAREN) + EQUAL + MLQSTRING + SEMI
)
precision = (
    Keyword("PRECISION")
    + BPAREN
...
variable_type = (
    Keyword("VARIABLE-TYPE") + BPAREN + QSTRING + EPAREN + EQUAL + QSTRING + SEMI
)
